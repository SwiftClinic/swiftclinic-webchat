...
(function(){
  try{
    ...
    function toggle(show){
      ...
      if(opened){
        input.focus();
        if(!welcomeShown && welcomeMessage && !appendedWelcomeThisOpen && msgs.childElementCount===0){ append('bot', welcomeMessage); appendedWelcomeThisOpen = true; welcomeShown = true; try{ sessionStorage.setItem(welcomeKey, '1') }catch(_){ } renderStarters(); }
    ...
    function renderStarters(){
      try{
        ...
        conversationStarters.forEach(function(label){
          ...
          chip.addEventListener('click', function(e){ e.preventDefault(); e.stopPropagation(); var text=String(label); input.value = text; hideStarters(); performSend(text); input.value=''; });
    ...
    function finishTypingWith(text){
      ...
      setTimeout(function(){
        ...
        if(t){
          ...
          msgs.scrollTop = msgs.scrollHeight;
          if(!startersShown && msgs.childElementCount>0){ renderStarters(); }
        } else {
