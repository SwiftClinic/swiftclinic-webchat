  ...
  try{
    ...
    function toggle(show){
      ...
      if(opened){
        ...
        // Inject starters under the first message if configured
        try{
          if(Array.isArray(conversationStarters) && conversationStarters.length>0 && !msgs.querySelector('#sc-starters-bubble')){
            var bubble = document.createElement('div');
            bubble.id = 'sc-starters-bubble';
            bubble.className = 'msg bot';
            var wrap = document.createElement('div'); wrap.className='md';
            conversationStarters.forEach(function(label){
              var chip = document.createElement('button'); chip.type='button'; chip.className='chip'; chip.textContent=String(label);
              chip.addEventListener('click', function(){ performSend(String(label)); hideStarters(); });
              wrap.appendChild(chip);
              var space = document.createTextNode(' '); wrap.appendChild(space);
            });
            bubble.appendChild(wrap); msgs.appendChild(bubble); msgs.scrollTop = msgs.scrollHeight;
          }
        }catch(_){ }
      } else {
    ...
    function hideStarters(){ try{ var s = msgs.querySelector('#sc-starters-bubble'); if(s){ s.remove(); } }catch(_){} }
    ...
    try{
      if(Array.isArray(conversationStarters) && conversationStarters.length>0 && startersEl){
        conversationStarters.forEach(function(label){
          var chip = document.createElement('button');
          chip.type = 'button';
          chip.className = 'chip';
          chip.textContent = String(label);
          chip.addEventListener('click', function(){ performSend(String(label)); });
